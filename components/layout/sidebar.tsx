...
import { LayoutGrid, Video, MessageSquare, CalendarDays, Settings, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Sidebar({ userName, initials }: Props) {
  const pathname = usePathname();
  const [collapsed, setCollapsed] = useState(false);
  const router = useRouter();

  return (
    <aside className={`flex-shrink-0 flex flex-col bg-[#FBF7EE] border-r border-[#E8E2D6] h-full transition-all duration-200 ${collapsed ? "w-[60px]" : "w-[220px]"}`}>
      <div className={`flex items-center py-6 ${collapsed ? "justify-center px-0" : "justify-between px-5"}`}>
        {!collapsed && <span className="font-archivo-black text-[#18150F] text-lg tracking-widest uppercase">LIFFT LABS</span>}
        <button onClick={() => setCollapsed(!collapsed)} className="text-[#6F6B60] hover:text-[#18150F] transition-colors">
      ...
      <div className="px-3 pb-4 flex flex-col gap-0.5">
        ...
        <div className={`flex items-center gap-3 px-3 py-2.5 ${collapsed ? "justify-center" : ""}`}>
          <div className="w-7 h-7 rounded-full bg-[#18150F] flex items-center justify-center flex-shrink-0">
            <span className="font-dm-sans text-white text-xs font-medium">{initials}</span>
          </div>
          {!collapsed && <span className="font-dm-sans text-sm text-[#18150F] truncate">{userName}</span>}
        </div>
      </div>
    </aside>
  );
